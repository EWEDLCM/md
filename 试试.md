...
| 路径 | 容器路径 (`/media`) | 结果 |
|-------------------------|---------------------|------|
| 主机                 | 无文件              | 容器内的 `/media` 显示主机 `/root/media` 中的文件 |
| 无文件                  | 有文件              | 容器内的 `/media` 被主机 `/root/media` 覆盖，容器内的文件不可见 |
| 有文件                  | 有文件              | 容器内的 `/media` 显示主机 `/root/media` 中的文件，容器内的原有文件被覆盖 |
| 无文件                  | 无文件              | 容器内的 `/media` 是一个空目录 |

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM3NzQ5MDcyMSw1MDMwNzcxMzUsLTE4ND
E1NzU2MTYsMTYzMDkyNDUzMV19
-->